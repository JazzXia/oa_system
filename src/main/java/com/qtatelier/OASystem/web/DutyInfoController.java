...
public class DutyInfoController {
    ...
    @ApiOperation(value = "下拉选项", notes = "下拉选项")
    @ApiImplicitParams({
            @ApiImplicitParam(name = "token", value = "令牌", paramType = "header", dataType = "String", required = true)
    })
    @SystemControllerLog(description = "下拉选项", optType = CodeBusiness.OPT_TYPE.SEARCH_CODE, moduleName = CodeBusiness.MODULE_NAME.DUTY_MODULE)
    @GetMapping("/dutylist")
    @UserLoginToken
    public ResultView findInfoByDeptId(String deptId, String token) {
        String logStr = "下拉选项";
        ResultView resultView = null;
        try {
            logger.info(logStr + "开始", token, deptId);
            List<DutyInfoRes> list = dutyInfoService.findInfoByDeptId(deptId);
            resultView = new ResultView(CodeEnum.SUCCESS, list);
        } catch (Exception e) {
            logger.error(logStr + "失败", e);
            resultView = new ResultView(CodeEnum.ERROR_500, e.getMessage());
